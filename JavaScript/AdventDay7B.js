(function() {
	...
	function CalculateSignalOf(wire) {
outerLoop:
		while(Object.keys(circuit).length > 0) {
			for(var w in circuit) {
				...
				if(circuit[w].signal != null && w == wire) {
					break outerLoop;
				}
			}
		}
		
		return circuit[wire].signal;
	}
	
	var a = CalculateSignalOf('a');
	
	for(var i in circuit) {
		circuit[i].signal = null;
	}
	
	circuit['b'].operands[0] = a.toString();

	return CalculateSignalOf('a');
})();